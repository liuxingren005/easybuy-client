  ...
  <div class="orders-page">
      ...
      <div v-for="order in tableData" :key="order.id" class="order-card">
        ...
        <div class="order-body">
          ...
          <div class="order-amount">
            ...
            <div v-if="Number(order.refundAmount) > 0" class="refunded-amount">
        ...
        <div class="order-footer">
          ...
          <el-button v-if="order.status === 2" size="small" type="warning" plain
    ...
    <el-dialog v-model="detailVisible" title="订单详情" width="680px">
      <div v-if="currentOrder" class="order-detail">
        ...
        <div class="detail-row" v-if="Number(currentOrder.refundAmount) > 0">
          <span class="label">累计退款：</span>
          <span class="refunded-text">¥{{ Number(currentOrder.refundAmount).toFixed(2) }}</span>
        </div>
        <div class="detail-row">
        ...
        <el-table :data="currentOrder.orderDetailList || []" border size="small">
          ...
          <el-table-column label="数量" width="120" align="center">
            <template #default="{ row }">
              {{ row.quantity }}
              <div v-if="row.refundQuantity > 0" class="refunded-qty">已退 {{ row.refundQuantity }}</div>
            </template>
          </el-table-column>
        ...
        <template v-if="refundRecords.length">
          ...
          <el-table :data="refundRecords" border size="small" class="refund-record-table"> <!-- 自定义样式类名 -->
            <el-table-column type="index" label="#" width="50" align="center" />
            <el-table-column prop="createTime" label="退款时间" width="160" align="center" />
    ...
    <el-dialog v-model="refundVisible" title="申请退款" width="680px">
      <div v-if="refundOrder" class="refund-dialog">
        <div class="refund-summary">
          ...
          <span v-if="Number(refundOrder.refundAmount) > 0">
            ｜ 已退 <b class="refunded-text">¥{{ Number(refundOrder.refundAmount).toFixed(2) }}</b>
          </span>
        </div>
        <el-table :data="refundItems" border size="small" class="refund-table">
          ...
          <el-table-column label="已退数量" width="90" align="center">
            <template #default="{ row }">{{ row.refundQuantity || 0 }}</template>
          </el-table-column>
...
const payTypeLabel = (payType, status) => {
  ...
  if (status === 2 || status === 4) return '未知'
  return '未支付'
}
...
const viewDetail = async (row) => {
  try {
    ...
    // 累计退款（部分退款/已退款）订单加载退款记录
    if (res.data.status === 4 || Number(res.data.refundAmount) > 0) {
      try {
...
// 明细剩余可退数量
const remainingQty = (row) => row.quantity - (row.refundQuantity || 0)
...
const allItemsFullyRefunded = computed(() => {
  ...
  return list.length > 0 && list.every(d =>
    (d.refundQuantity || 0) + (refundItems.value.find(i => i.id === d.id)?.refundQty || 0) >= d.quantity)
})
...
const estimatedRefundAmount = computed(() => {
  ...
  if (allItemsFullyRefunded.value) {
    return Number(refundOrder.value.cost) - Number(refundOrder.value.refundAmount || 0)
  }
...
/* 隐藏 */
.refund-record-table :deep(.el-table__inner-wrapper::before) {
  height: 0;
}
