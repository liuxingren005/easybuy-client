<template>
  <div class="addresses-page">
    ...
    <el-dialog v-model="dialogVisible" :title="isEdit ? '修改地址' : '新增地址'" width="520px" @close="resetForm">
      <el-form :model="form" :rules="rules" ref="formRef" label-width="90px"><!-- 数据源与规则池 -->
...
const { cascaderOptions, splitAddress } = useAddress()
...
const cascaderProps = {
  ...
  label: 'label',
  children: 'children'/* ,
  checkStrictly: true // 允许选择/显示任意层级（省、市非叶子节点） */
}
...
const rules = {
  ...
  address: [
    ...
    { min: 3, max: 50, message: '地址长度在 3-50 个字符', trigger: 'blur' }
  ]
}

// 根据地区编码数组获取名称路径
function getRegionLabels(codes) {
  if (!codes || codes.length === 0) return '' // 边界检查
  let labels = [] // 收集名称
  let level = cascaderOptions.value // 当前层级
  for (const code of codes) { // 遍历每个编码
    const found = level.find(n => n.value === code) // 当前层级匹配
    if (!found) break
    labels.push(found.label) // 收集名称
    level = found.children || [] // 进入下一层
  }
  return labels.join(' ') // 空格拼接
}
...
function openDialog(row) {
  if (row) {
    // 修改模式：反解析回填
    const { region, detail } = splitAddress(row.address)
    form.id = row.id
    form.region = region
    form.address = detail
