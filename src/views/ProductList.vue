  ...
  <div class="product-list-page">
    ...
    <div class="top-bar">
      <div class="top-inner">
        ...
        <HotSearchBar v-model="searchKeyword" @search="handleSearch" class="search-area" /> <!-- 当前分类搜索 -->
        <div class="cart-area">
          <CartHover />
        </div>
      </div>
    ...
    <div class="breadcrumb">
      <span class="crumb" @click="goToLevel(0)">全部</span>
      <span class="sep">›</span>
      <span class="crumb" v-if="category1Name" @click="goToLevel(1)">{{ category1Name }}</span>
      <span class="sep" v-if="category1Name">›</span>
      <span class="crumb" v-if="category2Name" @click="goToLevel(2)">{{ category2Name }}</span>
...
const loadProducts = async () => {
  ...
  try {
    ...
    const res = await productApi.search(params) // 调用分页接口 主用 ES 搜索
    // 更新
    productList.value = res.page?.list || []
    total.value = res.page?.total || 0
  } catch (e) {
    // 降级：使用分页查询，改用 MySQL 分页
    try {
      // 组装
      const params = {
        pageNum: pageNum.value,
      ...
      const res = await productApi.getPage(params) // 调用分页接口
      // 更新
      productList.value = res.page?.list || []
      total.value = res.page?.total || 0
    } catch (err) {
...
// 面包屑点击：分类层级跳转，利用 watch(route.query) 自动清空所有筛选（品牌/价格/类型/搜索词）
const goToLevel = (level) => {
  const query = {}
  if (level >= 1 && route.query.category1Id) query.category1Id = route.query.category1Id
  if (level >= 2 && route.query.category2Id) query.category2Id = route.query.category2Id
...
/* 
导航栏/首页分类/面包屑跳转（实时感知）
监听 URL query 变化：分类切换 / 搜索词变化 → 重置筛选条件并重新加载 
*/
watch(
  () => route.query,
  () => {
