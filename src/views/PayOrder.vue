  ...
  <div class="pay-page">
    ...
    <div class="pay-body">
      <div v-loading="loading" class="pay-content">
        <template v-if="order">
            ...
            <div class="method-list">
              ...
              <div :class="{ active: payType === 1 }" class="method-item alipay" @click="payType = 1">
                ...
                <div class="method-check">
                  <el-icon v-if="payType === 1" color="#ff6600">
                    <CircleCheck />
                  </el-icon>
                </div>
              </div>
              ...
              <div :class="{ active: payType === 2 }" class="method-item wechat" @click="payType = 2">
                ...
                <div class="method-check">
                  <el-icon v-if="payType === 2" color="#ff6600">
                    <CircleCheck />
                  </el-icon>
                </div>
              </div>
          ...
          <!-- 支付宝支付等待弹窗 -->
          <el-dialog v-model="alipayWaitingVisible" title="支付宝支付" width="400px" :close-on-click-modal="false"
            :show-close="false">
    ...
    <div v-if="showResult" class="pay-result">
      <div class="result-content">
        <el-icon v-if="paySuccess" class="result-icon success" size="64">
          <CircleCheck />
        </el-icon>
        <el-icon v-else class="result-icon fail" size="64">
          <CircleClose />
        </el-icon>
...
const alipayPay = async () => {
  ...
  const data = res.data || {}
  const html = data.html // 支付宝表单 HTML
  ...
  // 新窗口写入表单 HTML
  alipayWin = window.open('', '_blank') // 加载地址，新标签页
  if (alipayWin) {
    alipayWin.document.write(html) // 写入表单
    alipayWin.document.close()
  } else {
    ElMessage.warning('请允许浏览器弹出窗口以打开支付宝支付页面')
  }
  // 显示等待弹窗
  alipayWaitingVisible.value = true // 原窗口"等待支付"弹窗
  polling.value = true
...
const checkAlipayPaid = async () => {
  try {
    ...
    if (res.data && res.data.status === 2) {
      // 支付成功
      stopPolling() // 停掉定时器
      closeAlipayWin() // 关闭支付宝新窗口
      alipayWaitingVisible.value = false // 关闭"等待支付"弹窗
      order.value = res.data // 刷新订单
      paySuccess.value = true // 标记
      showResult.value = true
    } else {
  ...
  pollTimer = setInterval(async () => {
    ...
    try {
      ...
      if (data.tradeState === 'SUCCESS') {
        ...
        // 重新加载订单
        const orderRes = await orderApi.getById(order.value.id)
        order.value = orderRes.data // 刷新订单
